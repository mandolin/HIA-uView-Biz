...
| [Static transport-operation contract / 静态 transport-operation 契约](transport-operation.md) | Reviewed local read-operation descriptor, complete handler map, isolated dispatch, and no-HTTP boundary / 已审阅本地 read-operation descriptor、完整 handler map、隔离 dispatch 与无 HTTP 边界 |
| [Provider-port contract / Provider-port 契约](provider-port.md) | Explicit consumer-owned session, optional storage, read/write provider host, plain-data isolation, failure redaction, and bounded rollback / 显式 consumer-owned session、可选 storage、read/write provider host、plain-data 隔离、failure 脱敏与受限 rollback |
| [Async project-provider contract / 异步项目 provider 契约](async-provider.md) | Explicit injected Promise lifecycle, source policy, timeout/cancel/race isolation, finite read degradation, fixed write authority, and count-only observation / 显式注入 Promise 生命周期、source policy、timeout/cancel/race 隔离、有限 read 降级、固定 write authority 与仅计数 observation |
| [Checkout-first consumer contract / Checkout-first consumer 契约](checkout-first-consumer.md) | Explicit source-checkout manifest/profile, isolated consumer fixture, and local provider evidence / 显式 source-checkout manifest/profile、隔离 consumer fixture 与本地 provider 证据 |
| [Transport-operation descriptor schema / Transport-operation descriptor schema](schemas/transport-operation.descriptor.v1.schema.json) | Restricts exact static local-synchronous read-operation metadata / 限制精确静态 local-synchronous read-operation metadata |
| [Provider-port declaration schema / Provider-port declaration schema](schemas/provider-port.declaration.v1.schema.json) | Restricts version, ownership, kind, credential, execution, and rollback metadata / 限制版本、主责、分类、credential、execution 与 rollback metadata |
| [Neutral catalog transport descriptor example / 中性目录 transport descriptor 示例](examples/example.catalog-query-detail.local-transport.descriptor.json) | Declares the fixture's query/detail local read operations / 声明 fixture 的 query/detail 本地 read operation |
...
| [Transport-operation runtime API / Transport-operation runtime API](../api/transport-operation-runtime.md) | Executable static local operation descriptor/handler-map/dispatch boundary / 可执行静态本地 operation descriptor/handler-map/dispatch 边界 |
| [Provider-port runtime API / Provider-port runtime API](../api/provider-port-runtime.md) | Executable explicit provider declaration/host, redaction, and observation surface / 可执行显式 provider declaration/host、脱敏与 observation 表面 |
| [Async-provider runtime API / 异步 provider runtime API](../api/async-provider-runtime.md) | Executable async declaration/source-policy/host, Promise handle, cancellation, timeout, retry, degradation, and observation surface / 可执行 async declaration/source-policy/host、Promise handle、取消、timeout、retry、降级与 observation 表面 |
