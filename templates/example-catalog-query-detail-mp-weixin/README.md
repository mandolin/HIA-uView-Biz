...
The factory performs no package discovery, file loading, network access, backend selection, credential handling, dynamic import, script execution, or source fallback. Its output remains separate from the representative app profile and from all business/implementation manifests.

该工厂不执行 package discovery、文件加载、网络访问、后端选择、credential 处理、动态 import、脚本执行或 source fallback。其输出与代表性 app profile 以及所有 business/implementation manifest 保持分离。
