...
| `createCatalogQueryDetailMock(options)` | Returns explicit query/detail/session port providers, route projection, and route-action resolver / 返回显式 query/detail/session port provider、路由投影和路由 action 解析器 |
...
The package owns only neutral test entries and canonical fixture results. It reads no network, file, environment variable, storage, account, token, cookie, backend envelope, Directus collection, UI component, or HIA-uView dependency.
