...
| [Representative `mp-weixin` slice / 代表性 `mp-weixin` 纵切](representative-mp-weixin-slice.md) | Versioned app profile, explicit local source selection, full-stack acceptance path, registered presentation blocks, and layer ownership / 带版本 app profile、显式本地数据源选择、完整 stack 验收路径、已登记呈现区块与分层主责 |
| [Representative profile schema / 代表性 profile schema](schemas/representative-mp-weixin.profile.v1.schema.json) | Restricts source mode, initial page values, and compiled presentation block IDs / 限制数据源模式、初始分页值与已编译呈现区块 ID |
