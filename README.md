...
| `packages/provider-port-runtime/` | Explicit consumer-owned session, optional storage, read, and write provider host with plain-data isolation and bounded rollback / 显式 consumer-owned session、可选 storage、read、write provider host，以及 plain-data 隔离与受限 rollback |
| `packages/async-provider-runtime/` | Explicit injected Promise provider lifecycle, source policy, timeout/cancel/race isolation, bounded read degradation, and fixed write authority / 显式注入 Promise provider 生命周期、source policy、timeout/cancel/race 隔离、受限 read 降级与固定 write authority |
...
`npm test` runs the workspace/ROP gates and deterministic Node contract fixtures. With dependencies already installed from the committed lockfile, it performs no install and does not create a lockfile. See the [adoption guide / 采用指南](docs/adoption.md), [migration guide / 迁移指南](docs/migration.md), [lifecycle and selection-transition boundary / 生命周期与选择过渡边界](docs/lifecycle-transition.md), [doctor reference / Doctor 参考](docs/doctor.md), [support guide / 支持与反馈](docs/support.md), [local source trial / 本地 source trial](docs/source-trial.md), [development notes / 开发说明](docs/development.md), [release-quality candidate / 发布质量候选](docs/quality.md), [architecture overview / 架构概览](docs/architecture.md), [provider-port contract / provider-port 契约](docs/contracts/provider-port.md), [async project-provider contract / 异步项目 provider 契约](docs/contracts/async-provider.md), [representative slice contract / 代表性纵切契约](docs/contracts/representative-mp-weixin-slice.md), [application-template contract / 应用模板契约](docs/contracts/application-template.md), [contracts / 契约](docs/contracts/README.md), [provider-port API / provider-port API](docs/api/provider-port-runtime.md), and [async-provider API / 异步 provider API](docs/api/async-provider-runtime.md).

`npm test` 运行 workspace/ROP 门禁和确定性 Node 契约 fixture。在已按 committed lockfile 安装依赖后，它不执行安装，也不创建 lockfile。详见[采用指南](docs/adoption.md)、[迁移指南](docs/migration.md)、[生命周期与选择过渡边界](docs/lifecycle-transition.md)、[Doctor 参考](docs/doctor.md)、[支持与反馈](docs/support.md)、[本地 source trial](docs/source-trial.md)、[开发说明](docs/development.md)、[发布质量候选](docs/quality.md)、[架构概览](docs/architecture.md)、[provider-port 契约](docs/contracts/provider-port.md)、[异步项目 provider 契约](docs/contracts/async-provider.md)、[代表性纵切契约](docs/contracts/representative-mp-weixin-slice.md)、[应用模板契约](docs/contracts/application-template.md)、[契约](docs/contracts/README.md)、[provider-port API](docs/api/provider-port-runtime.md)和[异步 provider API](docs/api/async-provider-runtime.md)。
